...
const ProductDetail: React.FC<ProductDetailProps> = ({ product, onBack }) => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => setIsQtyAnimating(false), 300);
    return () => clearTimeout(timer);
  }, [quantity]);
          ...
          <div className="flex flex-col">
                  ...
                  {product.options.map((option) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        {selectedOption.label === option.label && (
                          <div className="bg-brand-500 text-white rounded-full p-0.5 animate-in fade-in zoom-in duration-200">
                ...
                <div className="flex items-center gap-4">
                  <div className="flex items-center border-2 border-slate-200 rounded-xl">
                    <button 
                      ...
                      className="p-3 hover:bg-slate-50 text-slate-600 transition-colors rounded-l-xl active:bg-slate-100"
                      disabled={quantity <= 1}
                    >
                      <Minus size={20} />
                    </button>
                    <span 
                      className={`w-12 text-center font-bold text-lg text-slate-900 transition-all duration-300 ${
                        isQtyAnimating ? 'scale-125 text-brand-600' : 'scale-100'
                      }`}
                    >
                      {quantity}
                    </span>
                    <button 
                      ...
                      className="p-3 hover:bg-slate-50 text-slate-600 transition-colors rounded-r-xl active:bg-slate-100"
                    >
                      <Plus size={20} />
                    </button>
                  </div>
                  <div className="flex-1 text-right">
                     ...
                     <p 
                       className={`text-3xl font-bold text-brand-600 transition-all duration-300 origin-right ${
                         isPriceAnimating ? 'scale-110 text-brand-500' : 'scale-100'
                       }`}
                     >
                       Rs. {totalPrice}
                     </p>
            ...
            <div className="mt-auto flex flex-col gap-3">
              ...
              <button 
                ...
                className="w-full bg-slate-900 text-white py-4 rounded-xl font-bold text-lg hover:bg-brand-600 transition-all shadow-lg hover:shadow-brand-500/20 hover:-translate-y-1 hover:scale-[1.03] active:scale-[0.98] flex items-center justify-center gap-2 group"
              >
